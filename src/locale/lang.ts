...
const LANG_STR: { [key: string]: string } = {
  ...
  feedback_syntaxerror_or_invalid_variables_en:
    "Syntax error or invalid variables in '$'.",
  feedback_syntaxerror_or_invalid_variables_de:
    "Syntaxfehler oder unzulässive Variablen in '$'.",
...
export function GET_STR(key: string, lang = 'en', redColor = true) {
  const complete_key = key + '_' + lang;
  //sellassert(complete_key in LANG_STR, "GET_STR(..): unknown key " + complete_key);  // TODO
  if (redColor)
    return '<span class="text-danger">' + LANG_STR[complete_key] + '</span>';
  else return LANG_STR[complete_key];
}
