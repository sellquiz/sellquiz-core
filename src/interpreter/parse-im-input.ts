...
import { SellQuiz, SellInput, SellInputElementType } from './quiz';
...
export class ParseIM_Input {
  ...
  parseIM_Input() {
    ...
    if (this.p.is('"')) {
      this.p.next();
      // gap question
      const gapTexts = []; // set of correct answers per gap
      let gapText = '';
    ...
    switch (sym.type) {
      ...
      case symtype.T_COMPLEX:
        ...
        /*} else {
          // -- real part --
          html +=
            '<input type="text" name="sell_input" value="" id="' +
            input.htmlElementId +
            '_real" size="' +
            inputWidth +
            '" placeholder=""> `+` ';
          ...
          html +=
            ...
            '_imag" size="' +
            inputWidth +
            '" placeholder=""> `i` '; // TODO: make i<->j configurable
          html += '<span id="' + input.htmlElementId_feedback + '"></span>';
        }*/
        break;
      ...
      case symtype.T_MATRIX_OF_FUNCTIONS:
        ...
        if (sym.type == symtype.T_MATRIX) {
          rows = this.p.resizableRows
            ? 2
            : SellLinAlg.mat_get_row_count(sym.value);
          cols = this.p.resizableCols
            ? 2
            : SellLinAlg.mat_get_col_count(sym.value);
        } else {
      ...
      case symtype.T_PROGRAMMING:
        ...
        if (this.p.generateInputFieldHtmlCode == false) {
          ...
          html +=
            '<div class="border p-0 m-0"><textarea class="form-control p-0" style="min-width: 100%;" id="' +
            input.htmlElementId +
            '" + rows="5"></textarea></div>';
          html += '<span id="' + input.htmlElementId_feedback + '"></span>';
        }*/
