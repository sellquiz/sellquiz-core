...
import { SellInput, SellInputElementType, SellQuiz } from './quiz';
...
export class ParseText {
  ...
  parseTitle(): void {
    ...
    while (!this.p.is('§EOL') && !this.p.is('§END')) {
      if (this.p.isIdent()) {
        ...
        this.p.next();
        if (this.p.isIdent()) {
          const tag = this.p.tk; // TODO: yet unused
          this.p.next();
  ...
  parseText(parsingHint = false): void {
    this.p.parseWhitespaces = true;
    while (!this.p.is('§END') && !this.p.is('§CODE_START')) {
      if (parsingHint && this.p.is('§EOL')) break;
      // end itemize, if applicable
      if (this.p.tk_col == 1 && !this.p.is('*')) this.endItemizeIfApplicable();
      // parse
      if (
        this.p.is('§EOL') &&
        this.p.singleMultipleChoiceFeedbackHTML.length > 0
      ) {
        this.p.q.html +=
      ...
      if (this.p.is('§EOL') && this.p.isItemizeItem) {
      ...
      else if (this.p.tk_col == 1 && this.p.is('*')) this.parseItemize();
      else if (this.p.is('`')) this.p.q.html += this.parseInlineListing();
      else if (this.p.is('```')) this.p.q.html += this.parseListing();
      else if (this.p.is('$')) this.p.imParser.parseInlineMath();
      else if (this.p.is('#'))
        ...
        const tokens = Lexer.splitStringAndKeepDelimiters(this.p.tk, [
          '__',
          '_',
        ]);
        this.p.next();
  ...
  //     "(" ("x"|expr) ")"
  //   | "[" ("x"|expr) "]";
  parseSingleMultipleChoice(isSingleChoice: boolean): void {
    // TODO: permutation!
    this.p.parseWhitespaces = false;
    let correct = false;
    if (isSingleChoice) this.p.terminal('(');
    else this.p.terminal('[');
    if (this.p.is('x')) {
      this.p.next();
      correct = true;
    } else if (!this.p.is(']') && !this.p.is(')') && !this.p.is(' ')) {
      this.p.codeParser.parseExpr();
      const v = this.p.q.stack.pop();
    ...
    const prefix = isSingleChoice ? '_sc_' : '_mc_';

    const sym = new SellSymbol(symtype.T_BOOL, correct);
    const symId = prefix + this.p.createUniqueID();
