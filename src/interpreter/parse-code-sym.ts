...
import * as Symbolic from './../math/symbolic';
...
export class ParseCodeSym {
  ...
  // symbolic_term =
  //   symbolic_term_add;
  parseSymbolicTerm(lhsSymbolIDs: Array<string>) {
    const symterm = new Symbolic.SellSymTerm(lhsSymbolIDs);
  ...
  // symbolic_term_expr =
  //   symbolic_term_add;
  parseSymbolicTerm_Expr(symterm: Symbolic.SellSymTerm) {
    this.parseSymbolicTerm_Add(symterm);
  }

  // symbolic_term_add =
  //   symbolic_term_mul { ("+"|"-") symbolic_term_mul };
  parseSymbolicTerm_Add(symterm: Symbolic.SellSymTerm) {
  ...
  parseSymbolicTerm_Mul(symterm: Symbolic.SellSymTerm) {
  ...
  parseSymbolicTerm_Pow(symterm: Symbolic.SellSymTerm) {
  ...
  //   | ID "(" [ expr { "," epxr } ] ")"
  //   | "-" symbolic_term_pow;
  parseSymbolicTerm_Unary(symterm: Symbolic.SellSymTerm) {
    if (this.p.is('(')) {
      this.p.terminal('(');
      ...
      if (id in this.p.q.symbols) {
        const symbol = this.p.q.symbols[id];
        if (symbol.type === symtype.T_REAL) symterm.pushConstant(symbol.value);
          ...
          if (this.p.is('(')) {
            ...
            while (!this.p.is(')') && !this.p.is('§EOF')) {
              ...
              const evalParameter = this.p.q.stack.pop();
            ...
            if (symbol.value.symbolIDs.length != evalParameters.length)
              this.p.err(
                "number of parameters does not match definition of function '" +
                  id +
                  "'",
              );
            const varValues: { [key: string]: any } = {};
