...
export class ParseIM {
  ...
  parseIM_Unary() {
    let html = '';
    if (this.p.is('#'))
      html += '`' + this.p.imInputParser.parseIM_Input() + '`';
    // '\`' ends and restarts math-jax env
      ...
      if (this.p.isIdent()) {
        ...
        if (
          this.p.tk in this.p.q.symbols &&
          this.p.q.symbols[this.p.tk].type == symtype.T_MATRIX
        )
      ...
      const m = LinAlg.SellLinAlg.mat_get_row_count(A);
      const n = LinAlg.SellLinAlg.mat_get_col_count(A);
      ...
      if (id in this.p.q.symbols) {
        if (this.p.q.symbols[id].type == symtype.T_MATRIX && this.p.is('[')) {
          ...
          if (this.p.is(':')) {
            ...
            this.p.next();
          } else this.p.err('expected integer for matrix row');
          // separator
          this.p.terminal(',');
          // col
          if (this.p.is(':')) {
            ...
            this.p.next();
          } else this.p.err('expected integer for matrix col');
          // end
          this.p.terminal(']');
          const symbol = this.p.q.symbols[id];
          if (symbol.type != symtype.T_MATRIX)
            this.p.err("'" + id + "' is not a matrix");
          const first_row = allRows ? 0 : i - 1;
          const last_row = allRows ? -1 : i - 1;
          const first_col = allCols ? 0 : j - 1;
          const last_col = allCols ? -1 : j - 1;
          const submat = LinAlg.SellLinAlg.mat_submatrix(
            symbol.value,
            first_row,
            last_row,
            first_col,
      ...
      this.p.next();
      html += '(';
      while (!this.p.is(')') && !this.p.is('§EOF')) html += this.parseIM_Expr();
      this.p.terminal(')');
      html += ')';
    } else if (this.p.is('{')) {
      this.p.next();
