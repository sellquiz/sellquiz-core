...
export class Lexer {
  ...
  static tokenize(s: string) {
    const tokens: Array<SellToken> = [];
    let str = '';
    let str_col = 1;
    let lineIdx = 1;
    let colIdx = 1;

    const allowUnderscoredelimiter =
  ...
  static splitStringAndKeepDelimiters(s: string, del: Array<string>) {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      for (let j = 0; j < del.length; j++) {
        const d = del[j];
        match = true;
        if (i + d.length > s.length) match = false;
        else {
