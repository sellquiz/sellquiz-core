...
export class ParseProg {
  ...
  parseProg(): void {
    let type = '';
    if (this.p.is('JavaBlock')) type = 'JavaBlock';
    else if (this.p.is('JavaMethod')) type = 'JavaMethod';
    else if (this.p.is('Python')) type = 'Python';
    else this.p.err('unknown programming type ' + this.p.tk);
    this.p.next();
    this.p.ident();
    const sym_id = this.p.id;
    this.p.terminal('§EOL');
    this.p.terminal('§CODE2_START');
    let code_given = '';
    const assert_list = [];
    while (this.p.is('given') || this.p.is('assert')) {
      if (this.p.is('given')) code_given += this.parseProgGiven();
      else assert_list.push(this.parseProgAssert());
    }
