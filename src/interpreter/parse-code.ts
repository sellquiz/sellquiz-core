...
import { SellSymTerm, SellSymTerm_Matrix } from './../math/symbolic';
...
export class ParseCode {
  ...
  parseCodeProp() {
    if (this.p.is('input')) this.p.next();
    else this.p.err('expected input');
    let isRows = false;
    if (this.p.is('rows')) {
      this.p.next();
      isRows = true;
    ...
    if (isRows) this.p.resizableRows = isResizable;
    else this.p.resizableCols = isResizable;
  }

  // code_hint =
  //   "?" text;
  parseCodeHint() {
    if (this.p.is('?')) this.p.next();
    else this.p.err('expected ?');
    const hintSym = this.p.q.lastParsedInputSymbol;
    if (hintSym == null)
      this.p.err(
        'Hint/explanation is forbidden, since there is no preceding input field',
      );
    const htmlLen = this.p.q.html.length;
  ...
  chooseFromSet(set: any) {
    let v;
    if (set.length == 4 && set[2].type === symtype.T_DOTS) {
      const lowerBound = set[0].value;
      const upperBound = set[3].value;
      const step = parseFloat(set[1].value) - lowerBound;
      v =
        Math.floor((Math.random() * (upperBound - lowerBound + step)) / step) *
          step +
        lowerBound;
    } else {
  ...
  parseAssign() {
    ...
    const lhsIDs: Array<string> = [];
    const lhsSymbolIDs: Array<string> = []; // e.g. for "f(x,y)", we call "x" and "y" symbols
    ...
    if (this.p.is(':=') || this.p.is('=')) {
      this.p.next();
      if (isFunction) this.p.codeSymParser.parseSymbolicTerm(lhsSymbolIDs);
      else this.parseExpr();
      const rhs = this.p.q.stack.pop();
      if (lhsMatrixIndexed) {
        ...
        const lhs = this.p.q.symbols[lhsIDs[0]];
    ...
    // choose element of right-hand side (rhs)
      ...
      if (this.p.is('MM')) {
        // matrix
        this.parseMatrixDef();
        const rhs = this.p.q.stack.pop(); // rhs is a matrix definition
        const m = rhs.value[0];
        const n = rhs.value[1];
        const set = rhs.value[2];
        const invertible = rhs.value[3];
        const symmetric = rhs.value[4];
        for (let k = 0; k < lhsIDs.length; k++) {
          ...
          do {
            ...
            for (let i = 0; i < m; i++) {
              for (let j = 0; j < n; j++) {
                const element = this.chooseFromSet(set.value);
            ...
            if (symmetric) {
              for (let i = 1; i < m; i++) {
                for (let j = 0; j < i; j++) {
                  const element = (value as math.Matrix).subset(
                    math.index(i, j),
                  );
                  value = (value as math.Matrix).subset(
                    math.index(j, i),
                    element,
                  );
                }
        ...
        // set
        if (this.p.is('{'))
          // TODO: move this to parseUnary!
          this.parseSet();
        else this.parseExpr();
        const rhs = this.p.q.stack.pop();
        if (rhs.type != symtype.T_SET) this.p.err('expected a set');
        // run until (optional) constrains are met
        const lex_backup = this.p.backupLexer();
        let ctr = 0;
        //while(true) {
        for (;;) {
          if (ctr > 1000)
          ...
          ctr++;
          for (let i = 0; i < lhsIDs.length; i++) {
            const value = this.chooseFromSet(rhs.value);
          ...
          if (this.p.is('with')) {
            this.p.next();
            this.parseExpr();
            const tos = this.p.q.stack.pop(); // tos := top of stack
  ...
  parseOr() {
    ...
    if (this.p.is('or')) {
      ...
      this.p.next();
      this.parseAnd();
      const o2 = this.p.q.stack.pop();
      const o1 = this.p.q.stack.pop();
  ...
  parseEqual() {
    ...
    if (this.p.is('==') || this.p.is('!=')) {
      ...
      if (o1.type == symtype.T_REAL && o2.type == symtype.T_REAL) {
        const isEqual = math.abs(o1.value - o2.value) < 1e-14;
        switch (op) {
          case '==':
        ...
        const isEqual = LinAlg.SellLinAlg.mat_compare_numerically(
          o1.value,
          o2.value,
        );
        switch (op) {
  ...
  parseAdd() {
    ...
    while (this.p.is('+') || this.p.is('-')) {
      ...
      if (o1.type == symtype.T_REAL && o2.type == symtype.T_REAL) {
        ...
        (o1.type == symtype.T_REAL || o1.type == symtype.T_COMPLEX) &&
        (o2.type == symtype.T_REAL || o2.type == symtype.T_COMPLEX)
      ) {
        switch (op) {
          case '+':
            this.p.pushSym(symtype.T_COMPLEX, math.add(o1.value, o2.value));
            break;
          case '-':
            this.p.pushSym(
              symtype.T_COMPLEX,
              math.subtract(o1.value, o2.value),
            );
            break;
        }
      } else if (o1.type == symtype.T_MATRIX && o2.type == symtype.T_MATRIX) {
        const o1_m = o1.value.size()[0];
        const o1_n = o1.value.size()[1];
        const o2_m = o2.value.size()[0];
        const o2_n = o2.value.size()[1];
      ...
      if (o1.type == symtype.T_REAL && o2.type == symtype.T_REAL) {
        switch (op) {
          ...
          case 'mod':
            ...
            this.p.pushSym(
              symtype.T_REAL,
              math.mod(math.round(o1.value), math.round(o2.value)),
            );
            break;
        }
      } else if (
        ...
        switch (op) {
          case '*':
            this.p.pushSym(
              symtype.T_COMPLEX,
              math.multiply(o1.value, o2.value),
            );
            break;
          case '/':
            this.p.pushSym(symtype.T_COMPLEX, math.divide(o1.value, o2.value));
            break;
          case 'mod':
        ...
        let o1_m, o1_n, o2_m, o2_n;
        switch (op) {
          case '*':
            o1_m = o1.value.size()[0];
            o1_n = o1.value.size()[1];
            o2_m = o2.value.size()[0];
            o2_n = o2.value.size()[1];
            if (o1_n != o2_m)
              this.p.err(
                'cannot multiply (' +
                  o1_m +
        ...
        switch (op) {
          ...
          case 'mod':
            ...
            this.p.pushSym(
              symtype.T_MATRIX,
              LinAlg.SellLinAlg.mat_mod(o1.value, o2.value),
            );
            break;
          default:
  ...
  parsePow() {
    ...
    while (this.p.is('^')) {
      ...
      if (o1.type == symtype.T_REAL && o2.type == symtype.T_REAL) {
        switch (op) {
          case '^':
            this.p.pushSym(symtype.T_REAL, math.pow(o1.value, o2.value));
            break;
        }
      } else if (
        (o1.type == symtype.T_REAL || o1.type == symtype.T_COMPLEX) &&
        ...
        switch (op) {
          case '^':
            this.p.pushSym(symtype.T_COMPLEX, math.pow(o1.value, o2.value));
            break;
        }
      } else if (
        o1.type == symtype.T_MATRIX &&
        o2.type == symtype.T_MATRIX_TRANSPOSE
      ) {
        switch (op) {
          case '^':
            this.p.pushSym(symtype.T_MATRIX, math.transpose(o1.value));
            break;
        }
      } else this.p.err("types not compatible for '" + op + "'");
    }
  ...
  parseUnary() {
    if (this.p.is('-')) {
      this.p.next();
      this.parseUnary();
      const o = this.p.q.stack.pop();
      if (o.type == symtype.T_REAL) this.p.pushSym(symtype.T_REAL, -o.value);
      else this.p.err("unary '-' must be followed by type real");
      ...
      this.p.q.stack.push(
        new SellSymbol(symtype.T_COMPLEX, math.complex(0, 1)),
      );
      this.p.next();
    } else if (this.p.is('T')) {
      ...
      const sym = this.p.q.symbols[id];
      if (sym.type == symtype.T_FUNCTION && this.p.is('(')) {
        this.p.next();
        this.parseAdd();
        const args = [this.p.q.stack.pop()];
        while (this.p.is(',')) {
          this.p.next();
          this.parseAdd();
          args.push(this.p.q.stack.pop());
        }
        const term: SellSymTerm = sym.value;
        const termSymIDs = term.symbolIDs;
        if (termSymIDs.length != args.length)
          this.p.err(
            'number of arguments does not correspond to function definition',
          );
        const args_dict: { [key: string]: any } = {};
        for (let i = 0; i < termSymIDs.length; i++) {
          if (args[i].type != symtype.T_REAL)
            this.p.err('all arguments must be scalar and real valued');
          args_dict[termSymIDs[i]] = args[i].value;
        }
        const v = term.eval(args_dict);
        const v_sym = new SellSymbol(symtype.T_REAL, v);
      ...
      this.parseExpr();
      this.p.terminal(')');
    } else this.p.err("expected unary, got '" + this.p.tk + "'");
    // postfix
  ...
  parseMatrix() {
    ...
    while (this.p.is('[') || this.p.is(',')) {
      ...
      while (!this.p.is(']') && !this.p.is('§EOF')) {
        ...
        this.parseExpr();
        const element = this.p.q.stack.pop();
        if (
          element.type != symtype.T_REAL &&
          element.type != symtype.T_FUNCTION
        )
          this.p.err('matrix element must be of type real or function');
        if (elementsType == null) elementsType = element.type;
    ...
    let matrix = null;
    const matrixType =
      elementsType == symtype.T_REAL
        ? symtype.T_MATRIX
        : symtype.T_MATRIX_OF_FUNCTIONS;
    if (matrixType == symtype.T_MATRIX) {
      matrix = math.zeros(rows, cols);
      sellassert(elements.length == rows * cols);
      for (let i = 0; i < rows; i++)
        for (let j = 0; j < cols; j++)
          matrix = LinAlg.SellLinAlg.mat_set_element(
            matrix as math.Matrix,
            i,
            j,
            elements[i * cols + j],
          );
  ...
  parseFunctionCall() {
    ...
    while (!this.p.is(')') && !this.p.is('§EOL') && !this.p.is('§EOF')) {
      ...
      if (functionName == 'integrate' && p.length == 1) {
        // second parameter of "integrate" must be a string
        this.p.ident();
        p.push(this.p.id);
      } else {
        this.parseExpr();
    ...
    if (functionName === 'abs') {
      if (
        p.length != 1 ||
        (p[0].type != symtype.T_REAL && p[0].type != symtype.T_COMPLEX)
      )
      ...
      const v = math.sqrt(p[0].value);
      const isComplex = math.typeOf(v) == 'Complex';
      this.p.pushSym(isComplex ? symtype.T_COMPLEX : symtype.T_REAL, v);
    } else if (
      ['sin', 'asin', 'cos', 'acos', 'tan', 'atan'].includes(functionName)
    ) {
      ...
      switch (functionName) {
        case 'sin':
          v = math.sin(p[0].value);
          break;
        case 'asin':
          v = math.asin(p[0].value);
          break;
        case 'cos':
          v = math.cos(p[0].value);
          break;
        case 'acos':
          v = math.acos(p[0].value);
          break;
        case 'tan':
          v = math.tan(p[0].value);
          break;
        case 'atan':
          v = math.atan(p[0].value);
          break;
        default:
      ...
      if (
        p.length != 2 ||
        p[0].type != symtype.T_REAL ||
        p[1].type != symtype.T_REAL ||
        !this.p.isNumberInt(p[0].value) ||
        !this.p.isNumberInt(p[1].value)
      )
        this.p.err("signature must be 'binomial(int,int)'");
      this.p.pushSym(
        symtype.T_REAL,
        math.combinations(math.round(p[0].value), math.round(p[1].value)),
      );
      ...
      if (
        p.length != 4 ||
        p[0].type != symtype.T_FUNCTION ||
        typeof p[1] !== 'string' ||
        p[2].type != symtype.T_REAL ||
        p[3].type != symtype.T_REAL
      )
        this.p.err(
          "signature must be 'integrate(function, string, real, real)'",
        );
      const v = p[0].value.integrateNumerically(p[1], p[2].value, p[3].value);
      const precision = 0.001;
      this.p.pushSym(symtype.T_REAL, v, precision);
    } else if (
      ['det', 'rank', 'inv', 'eigenvalues_sym', 'triu', 'norm2'].includes(
        functionName,
      )
    ) {
      ...
      if (functionName === 'det')
        ...
        const eigs = (math.eigs(p[0].value).values as any)._data;
        const set = [];
        ...
        this.p.pushSym(
          symtype.T_MATRIX,
          LinAlg.SellLinAlg.mat_triu(p[0].value),
        );
      ...
      if (
        p.length != 2 ||
        p[0].type != symtype.T_MATRIX ||
        p[1].type != symtype.T_MATRIX
      )
      ...
      if (LinAlg.SellLinAlg.mat_get_col_count(p[1].value) != 1)
        this.p.err("signature must be 'dot(columnVector,columnVector)'");
      if (
        LinAlg.SellLinAlg.mat_get_row_count(p[0].value) !=
        LinAlg.SellLinAlg.mat_get_row_count(p[1].value)
      )
        this.p.err("vectors in 'dot(..)' must have equal length");
      this.p.pushSym(
        symtype.T_REAL,
        LinAlg.SellLinAlg.mat_vecdot(p[0].value, p[1].value),
      );
      ...
      this.p.pushSym(
        symtype.T_MATRIX,
        LinAlg.SellLinAlg.mat_veccross(p[0].value, p[1].value),
      );
      ...
      if (
        LinAlg.SellLinAlg.mat_get_col_count(p[0].value) !=
        LinAlg.SellLinAlg.mat_get_row_count(p[0].value)
      )
      ...
      this.p.pushSym(
        symtype.T_MATRIX,
        LinAlg.SellLinAlg.linsolve(p[0].value, p[1].value),
      );
      ...
      for (let i = 0; i < p[0].value.length; i++) {
        if (p[0].value[i].value < v) v = p[0].value[i].value;
      }
      this.p.pushSym(symtype.T_REAL, v);
  ...
  parseMatrixDef() {
    ...
    // number of rows
    this.parseExpr();
    const m = this.p.q.stack.pop();
    if (m.type !== symtype.T_REAL || !this.p.isNumberInt(m.value))
      this.p.err('expected integer for the number of rows');
    const rows = math.round(m.value);
    if (rows <= 0)
      this.p.err(
        "number of matrix cols must be > 0 (actually is '" + rows + "')",
      );
    // times
    this.p.terminal('x');
    // number of columns
    this.parseExpr();
    const n = this.p.q.stack.pop();
    if (n.type !== symtype.T_REAL || !this.p.isNumberInt(n.value))
      this.p.err('expected integer for the number of columns');
    const cols = math.round(n.value);
    if (cols <= 0)
      this.p.err(
        "number of matrix rows must be > 0 (actually is '" + cols + "')",
      );
    // set
    this.p.terminal('|');
    this.parseExpr();
    const set = this.p.q.stack.pop();
    ...
    this.p.terminal(')');
    // create symbol
    this.p.pushSym(symtype.T_MATRIX_DEF, [
      rows,
      cols,
      set,
      invertible,
  ...
  parseSet() {
    this.p.terminal('{');
    let idx = 0;
    const sym = new SellSymbol(symtype.T_SET);
    sym.value = [];
    let hasDot = false;
    while (!this.p.is('}') && !this.p.is('§EOF')) {
      if (idx > 0) this.p.terminal(',');
      this.parseExpr();
      const symi = this.p.q.stack.pop();
      sym.value.push(symi);
      if (symi.type !== symtype.T_REAL) {
        if (idx == 2 && symi.type === symtype.T_DOTS) hasDot = true;
        else if (symi.type === symtype.T_COMPLEX)
          sym.type = symtype.T_COMPLEX_SET;
        else this.p.err('set must consist of real values only');
      }
      idx++;
    }
    if ((hasDot && idx != 4) || (hasDot && sym.type == symtype.T_COMPLEX_SET))
      this.p.err(
        "if set contains '...', then it must have 4 real-valued elements",
      );
