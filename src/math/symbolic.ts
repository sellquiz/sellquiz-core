...
export class SellSymTerm {
  ...
  constructor(symbolIDs: Array<string> = []) {
  ...
  importTerm(str: string): boolean {
    const n = math.parse(str);
    return this.importMathJsTermJsRecursively(n);
  }

  importMathJsTermJsRecursively(node: any) {
    ...
    switch (node.type) {
      ...
      case 'ParenthesisNode':
        ...
        if (node.fn == 'unaryMinus') {
          ...
          console.log(
            'warning: SellSymTerm::importMathJsTermJsRecursively(..): unknown/unimplemented operation ' +
              node.op,
          );
          return false;
        }
      ...
      case 'FunctionNode':
        if (
          ...
          console.log(
            'warning: SellSymTerm::importMathJsTermJsRecursively(..): unknown/unimplemented function ' +
              node.name,
          );
          return false;
        }
      ...
      default:
        console.log(
          'warning: SellSymTerm::importMathJsTermJsRecursively(..): unknown/unimplemented node type ' +
            node.type,
        );
        return false;
    }
    ...
    if (op1.type == 'const' && op2.type == 'const') {
      ...
      switch (type) {
        ...
        case '/':
          ...
          break;
        case '^':
          res = math.pow(op1.v, op2.v);
          break;
        default:
          console.log(
            'unimplemented: SellSymTerm:pushBinaryOperation(..): operator ' +
              type,
          );
      }
  ...
  appendVariableSet(v: any, v_new: any) {
  ...
  getVariables(element: any = null) {
    ...
    switch (element.type) {
      ...
      default:
        console.log(
          'unimplemented: SellSymTerm:getVariables(..): ' + element.type,
        );
    }
    return v;
  }

  getOdeOrder(element: any = null) {
    ...
    switch (element.type) {
      ...
      case 'binop':
        ...
        break;
      case 'fct2':
        if (element.v[0] == 'diff_ode') o = math.max(o, 1);
        else if (element.v[0] == 'diff2_ode') o = math.max(o, 2);
        break;
    }
    return o;
  }

  optimizeOdeConstants(element: any = null) {
    let root = false;
    let e, e1, e2;
    if (element == null) {
      root = true;
  ...
  searchForForbiddenODESecondOrderSubterms(element: any = null) {
    const v: Array<any> = [];
    let v1, v2;
    if (element == null) {
      element = this.stack[0];
    ...
    switch (element.type) {
      ...
      default:
        console.log(
          'unimplemented: SellSymTerm:searchForForbiddenODESecondOrderSubterms(..): ' +
            element.type,
        );
    }
    console.log('TEST:');
  ...
  getOperator(element: any): string {
    let op = '';
    if (element.type == 'uniop' || element.type == 'binop') op = element.v[0];
    return op;
  }

  getOperatorPrecedence(op: any): number {
    let p = 0;
    switch (op) {
      case '+':
        p = 10;
      ...
      case '-':
        p = 10;
        break; // TODO: this was 11
      case '*':
        p = 20;
        break;
      case '/':
        p = 20;
        break; // TODO: this was 21
      case '^':
        p = 30;
        break;
      case '':
      ...
      default:
        console.log(
          'unimplemented: SellSymTerm:getOperatorPrecedence(..): op=' + op,
        );
    }
    return p;
  }

  toString(element: any = null): string {
    let s = '';
    let root = false;
    if (element == null) {
      root = true;
    ...
    switch (element.type) {
      ...
      case 'const':
        ...
        if (math.abs(element.v - 3.141592653589793) < 1e-12) s = 'pi';
        break;
      case 'uniop':
      ...
      case 'ode_fun':
        ...
        break;
      case 'binop':
        c1 = this.toString(element.v[1]);
        c2 = this.toString(element.v[2]);
        op = element.v[0];
        op_precedence = this.getOperatorPrecedence(op);
        op1 = this.getOperator(element.v[1]);
        op1_precedence = this.getOperatorPrecedence(op1);
        op2 = this.getOperator(element.v[2]);
        op2_precedence = this.getOperatorPrecedence(op2);

        if (op1_precedence < op_precedence) c1 = '(' + c1 + ')';
        else if (op === '/') c1 = '(' + c1 + ')';

        if (op2_precedence < op_precedence /*|| op2.type == 'uniop'*/)
          c2 = '(' + c2 + ')';
        else if (c2.startsWith('-')) c2 = '(' + c2 + ')';
        else if (op === '-' || op === '/') {
      ...
      case 'fct1':
        ...
        if (name === 'exp' && arg.length < 5) s = 'e^(' + arg + ')';
        else s = name + '(' + arg + ')';
        break;
      case 'fct2':
        name = element.v[0];
        ...
        break;
      default:
        console.log('unimplemented: SellSymTerm:toString(..): ' + element.type);
    }
  ...
  derivate(variable: any /*id*/, element: any = null) {
    let u, v, op, name, p1;
    const isRoot = element == null;
    if (isRoot) element = this.stack[0];
    switch (element.type) {
      case 'var':
        element.deriv = new SellSymTermElement(
          'const',
          element.v == variable ? 1 : 0,
        );
        break;
      case 'const':
      ...
      case 'uniop':
        ...
        switch (op) {
          ...
          default:
            console.log(
              'unimplemented: SellSymTerm:derivate(..): uniop: ' + op,
            );
        }
        break;
      case 'binop':
        ...
        switch (op) {
          ...
          case '-':
            // f  = u + v
            // f' = u' + v'
            element.deriv = new SellSymTermElement('binop', [
              op,
              u.deriv,
              v.deriv,
            ]);
          ...
          case '^':
            ...
            if (v.type !== 'const')
              console.log(
                'unimplemented: SellSymTerm:derivate(..): derivation of u^v with v != const',
              );
            element.deriv = new SellSymTermElement('binop', [
              '*',
              u.deriv,
          ...
          default:
            console.log(
              'unimplemented: SellSymTerm:derivate(..): binop: ' + op,
            );
        }
        break;
      case 'fct1':
        ...
        switch (name) {
          ...
          case 'cos':
            // f = cos(p1)
            // f' = - p1' * sin(p1)
            element.deriv = new SellSymTermElement('uniop', [
              '-',
              new SellSymTermElement('binop', [
                '*',
                p1.deriv,
          ...
          default:
            console.log(
              'unimplemented: SellSymTerm:derivate(..): fct1: ' + name,
            );
        }
        break;
      default:
        console.log('unimplemented: SellSymTerm:derivate(..): ' + element.type);
    }
    if (isRoot) {
      const t = new SellSymTerm();
  ...
  eval(var_values: any /*dict*/, element: any = null) {
    let res: any, u, v, op, name, param, param1, param2;
    let t, t_id, t_deriv_var_id, t_deriv;
    res = 0;
    ...
    switch (element.type) {
      case 'var':
        if (element.v in var_values) {
          ...
          console.log(
            'SellSymTerm:eval(..): variable ' + element.v + ' has no value!',
          );
          return 0;
        }
        break;
      ...
      case 'uniop':
        ...
        switch (op) {
          case '-':
            ...
            break;
          default:
            console.log('unimplemented: SellSymTerm:eval(..): uniop: ' + op);
        }
        break;
      case 'binop':
        u = this.eval(var_values, element.v[1]);
        ...
        switch (op) {
          ...
          case '^':
            ...
            break;
          default:
            console.log('unimplemented: SellSymTerm:eval(..): binop: ' + op);
        }
        break;
      case 'fct1':
        name = element.v[0];
        ...
        switch (name) {
          ...
          case 'sqrt':
            ...
            break;
          default:
            console.log('unimplemented: SellSymTerm:eval(..): fct1: ' + name);
        }
        break;
      case 'fct2':
        name = element.v[0];
        ...
        switch (name) {
          ...
          case 'diff2_ode':
            t = null;
            t_id = element.v[1].v.id;
            if (t_id in var_values) t = var_values[t_id];
            else console.log('SellSymTerm:eval(..): unknown term ' + t_id);
            t_deriv_var_id = element.v[2].v;
            t_deriv = t.derivate(t_deriv_var_id);
            if (name === 'diff2_ode')
            ...
            /*let order = 1;
                        ...
                        /*if(element.v[1].v.type === "function") {
                            ...
                            if(t_id in var_values)
                                ...
                                console.log("SellSymTerm:eval(..): unknown term " + t_id);
                        }
                        let t_deriv_var_id = element.v[2].v;
                        let t_deriv = t.derivate(t_deriv_var_id);
                        res = t_deriv.eval(var_values);*/
      ...
      case 'ode_fun':
        ...
        break;
      default:
        console.log('unimplemented: SellSymTerm:eval(..): ' + element.type);
    }
    return res;
  }

  optimize(element: any = null) {
    let op, u, v;
    const isRoot = element == null;
    if (isRoot) element = this.stack[0];
    if (element.type === 'uniop') {
      ...
      if (u.type === 'const' && v.type === 'const') {
        ...
        switch (op) {
          ...
          default:
            console.log(
              'unimplemented: SellSymTerm:optimize(..): binop: ' + op,
            );
        }
        //console.log(element)
      }
      // 0, if u or v zero
        ...
        op === '*' &&
        ((u.type === 'const' && u.v == 0) || (v.type === 'const' && v.v == 0))
      ) {
        element.type = 'const';
        element.v = 0;
      }
  ...
  integrateNumerically(varId: string, a: number, b: number): number {
    ...
    let res = 0;
    for (let x = a; x < b; x += h) {
      const f1 = this.eval({ varId: x });
      const f2 = this.eval({ varId: x + h });
      res += ((f1 + f2) / 2) * h;
    }
    return res;
  }

  compareWithStringTerm(t: any, listOfSymbols: Array<any> = []) {
    ...
    const epsilon = 1e-6; // TODO: configure espilon

    for (let i = 0; i < n; i++) {
      const scope: { [key: string]: any } = {};
      for (let j = 0; j < vars.length; j++) scope[vars[j]] = math.random(l, u);
      //console.log("scope")
      //console.log(scope)
      //console.log("t")
      //console.log(t)
      const res = this.eval(scope);
      if (res > 1000.0) {
      ...
      if (math.abs(res - t_res) >= epsilon) return false;
    }
...
export class SellSymTerm_Matrix {
  ...
  constructor(m: number, n: number, elements: Array<any> = []) {
    this.m = m;
    this.n = n;
