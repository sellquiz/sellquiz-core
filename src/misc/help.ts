...
/*export function getHtmlChildElementRecursive(parentElement : Element, childID : string) : Element {
    for(let i=0; i<parentElement.children.length; i++) {
        if(parentElement.children[i].id == childID)
            ...
            const e = getHtmlChildElementRecursive(parentElement.children[i], childID);
            if(e != null)
                return e;
        }
    }
